...
import { updateDate } from '@/utils/notification/download'
...
Object.defineProperty(stats, 'contents-count', {
  get() {
    ...
    return Object.values(stats)
      .slice(1, -3)
      .reduce<number>((sum, value) => sum + (value as number), 0)
  },
  set() {},
})
...
const startMeasuring = function () {
  ...
  for (const key of keys) {
    const setter = Object.getOwnPropertyDescriptor(stats, key)?.set
  ...
  const timer = setInterval(function () {
    ...
    for (const [key, element] of elements) {
      const getter = Object.getOwnPropertyDescriptor(stats, key)?.get
...
const startDownload = async function () {
  ...
  const addExclusion = function (downloadContext: DownloadContext) {
    const context = castContentContext(downloadContext)
    exclusionList.add(context.hash)
  }

  scraper = new Scraper(async function (downloadContext) {
    const context = castContentContext(downloadContext)
...
const testScraping = async function () {
  ...
  scraper = new Scraper(function (downloadContext) {
    const context = castContentContext(downloadContext)
    stats['pending-count']++
    insert.appendContent(context)
  }, insert.appendError)
