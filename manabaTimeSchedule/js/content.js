...
import {registerDragEventDict, registerOverEventArray, registerOverEventDict } from "./courseCardDrag.js";
...
export function main(){
    ...
    //courseCardを新しく生成
    let courseCards = document.getElementsByClassName("coursecard-c");
    
    let newCourseCardDict = {};
    for (let i=0;i<courseCards.length;i++){
        let newCard = createCourseCard(courseCards[i]);
        newCourseCardDict[newCard.id] = newCard ;
    };
    if(Object.keys(newCourseCardDict) == 0){return};//情報が取得できなかったら終了
    ...
    function onGot(setting) {
        ...
        if (Object.keys(setting).length>4){
            //時間割の作成
            ...
            let i = 0;
            for(let key in newCourseCardDict){
        ...
        //ドロップ先の実装、boxes
        let boxes = document.getElementsByClassName("courseCardFrame");
